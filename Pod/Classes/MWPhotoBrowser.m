...
@implementation MWPhotoBrowser {
    UIBarButtonItem *_bigFixedSpace;
}
...
- (void)viewDidLoad {
    ...
    if (self.displayNavArrows) {
        NSString *arrowPathFormat = @"Frameworks/MWPhotoBrowser.framework/MWPhotoBrowser.bundle/UIBarButtonItemArrow%@";
...
- (void)performLayout {
    ...
    _bigFixedSpace = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFixedSpace target:self action:nil];
    [self calculateToolButtonSpace];
    ...
    if (_previousButton && _nextButton && numberOfPhotos > 1) {
        ...
        [items addObject:_bigFixedSpace];
...
- (void)calculateToolButtonSpace {
    CGFloat screenWidth = [UIScreen mainScreen].bounds.size.width;
    _bigFixedSpace.width = screenWidth - 148;
}
...
- (void)videoFinishedCallback:(NSNotification*)notification {
    ...
    // Dismiss
    if (_currentVideoPlayerViewController.player.status == AVPlayerItemStatusFailed) {
